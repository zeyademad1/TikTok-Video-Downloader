...
import com.zeyad.tiktokdownloader.domain.TikTokRepository
import com.zeyad.tiktokdownloader.util.Utils
import com.zeyad.tiktokdownloader.util.Utils.AUDIOS
import com.zeyad.tiktokdownloader.util.Utils.VIDEOS
...
class DownloadViewModel : ViewModel() {
    private val tiktokRepo: TikTokRepository = TikTokRepository()
    ...
    fun downloadVideo(filename: String, downloadUrlOfVideo: String, context: Context) {
        try {
            ...
            request.setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI or DownloadManager.Request.NETWORK_MOBILE)
                .setAllowedOverRoaming(false).setTitle(filename).setMimeType("video/*")
    ...
    fun downloadAudio(filename: String, downloadUrlOfVideo: String, context: Context) {
        try {
            ...
            val request = DownloadManager.Request(downloadUri)
            request.setAllowedNetworkTypes(DownloadManager.Request.NETWORK_WIFI or DownloadManager.Request.NETWORK_MOBILE)
                .setAllowedOverRoaming(false).setTitle(filename).setMimeType("audio/*")
                .setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
    ...
    fun showAlertDialog(heading: String, subHeading: String, context: Context) {
        AlertDialog.Builder(context).setTitle(heading).setMessage(subHeading)
    ...
    suspend fun getVideosFromFolder(context: Context, folderPath: String): ArrayList<MediaModel> =
        withContext(
            Dispatchers.IO
        ) {
            ...
            context.contentResolver.query(queryUri, projection, selection, selectionArgs, sortOrder)
                ?.use { cursor ->
                    while (cursor.moveToNext()) {
                        ...
                        videoList.add(
                            MediaModel(
                                displayName, contentUri, formatDuration(duration), formatSize(size)
                            )
                        )
                        Log.d("MainActivity" ,"content uri $contentUri.toString()")
                    }
                }
            return@withContext videoList
        }

    suspend fun getAudiosFromFolder(context: Context, folderPath: String): ArrayList<MediaModel> =
        withContext(
            Dispatchers.IO
        ) {
            ...
            context.contentResolver.query(queryUri, projection, selection, selectionArgs, sortOrder)
                ?.use { cursor ->
                    while (cursor.moveToNext()) {
                        ...
                        val contentUri: Uri = ContentUris.withAppendedId(
                            MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, videoId
                        )
                        audioList.add(
                            MediaModel(
                                displayName, contentUri, formatDuration(duration), formatSize(size)
